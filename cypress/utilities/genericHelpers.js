class GenericHelpers {
    ...
    focusDrag(sourceElement,targetElement){
        cy.get(sourceElement).trigger('mousedown', { button: 0 }, { force: true }).trigger('mousemove', 200, -200, { force: true });
        cy.get(targetElement).click().trigger('mouseup', { force: true }),then(function(){
            cy.log('Element is dragged from ' + sourceElement + ' to ' + targetElement)
        }, function(err){
            cy.log('--->Error: Unable to drag Element due to' + err);
        });
